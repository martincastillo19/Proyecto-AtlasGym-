...
app.put("/clientes/actualizar", (req, res) => {
  ...
  if (!clienteActualizado.correoOriginal) {
    return res.status(400).send("Falta correoOriginal para identificar cliente.");
  }
  fs.readFile(CLIENTES_PATH, "utf8", (err, data) => {
    ...
    const nuevasLineas = lineas.map((linea) => {
      const [nombre, apellido, rut, correo, ultimoPago, fechaVencimiento] = linea.split("|");
      if (correo === clienteActualizado.correoOriginal) {
        // Asegura que si antes no tenía fechaVencimiento, se mantenga el formato
        return `${clienteActualizado.nombre}|${clienteActualizado.apellido}|${clienteActualizado.rut}|${clienteActualizado.correo}|${clienteActualizado.ultimoPago}|${fechaVencimiento || ""}`;
      }
      return linea;
    });
...
// ACTUALIZAR FECHA DE PAGO Y VENCIMIENTO
// ACTUALIZAR FECHA DE PAGO Y VENCIMIENTO
app.put("/clientes/:rut/vencimiento", (req, res) => {
  // LOGS DE DEPURACIÓN
  console.log("Llego una petición a /clientes/:rut/vencimiento");
  console.log("req.params:", req.params);
  console.log("req.body:", req.body);

  const { rut } = req.params;
  const { nuevoPago } = req.body; // debe venir como dd/mm/yy

  // Si no viene nuevoPago, corta
  if (!nuevoPago) {
    console.log("No se recibió nuevoPago en el body");
    return res.status(400).send("Falta nuevoPago");
  }

  // Calcular la fecha de vencimiento (30 días después)
  const [dia, mes, anio] = nuevoPago.split('/');
  const fechaPago = new Date(2000 + parseInt(anio), parseInt(mes) - 1, parseInt(dia));
  const fechaVenc = new Date(fechaPago);
  fechaVenc.setDate(fechaPago.getDate() + 30);

  // Formatear a dd/mm/yy
  const vencimiento = [
    fechaVenc.getDate().toString().padStart(2, '0'),
    (fechaVenc.getMonth() + 1).toString().padStart(2, '0'),
    fechaVenc.getFullYear().toString().slice(-2)
  ].join('/');

  fs.readFile(CLIENTES_PATH, "utf8", (err, data) => {
    if (err) {
      console.log("Error al leer clientes:", err);
    ...
    const nuevasLineas = lineas.map((linea) => {
      ...
      if (partes[2] === rut) { // partes[2] es el RUT
        partes[4] = nuevoPago;       // Actualiza ultimoPago
        partes[5] = vencimiento;     // Actualiza fechaVencimiento
        modificado = true;
      }
      return partes.join("|");
    });
    ...
    fs.writeFile(CLIENTES_PATH, nuevasLineas.join("\n") + "\n", (err2) => {
      if (err2) {
        console.log("Error al guardar clientes:", err2);
        return res.status(500).send("Error al guardar clientes.");
      }
      console.log("Cliente actualizado correctamente:", rut, "Nuevo pago:", nuevoPago, "Vencimiento:", vencimiento);
      res.json({ exito: true, nuevoPago, vencimiento });
    });
  });
});
